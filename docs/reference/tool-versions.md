...
| FastAPI / Starlette | 0.141.1 / 1.6.0 | App and TestClient checks; Starlette security floor is 1.3.1 |
| AnyIO | 4.14.2 | Locked TestClient compatibility |
| Pillow | 12.3.0 | Decode, metadata, size and persistence |
| Node.js | 24.11.1 | Direct MCP protocol check |
...
The direct MCP protocol check exposed 24 tools and accessed only the isolated local login page,
without personal browser state. This was **not a TRAE-host run**. The pinned release supports `--codegen python`;
...
The pinned `browser_type` schema requires `target`; an earlier check using `ref` failed before the
correct direct-MCP call passed. Direct MCP output is exploratory host material, not independently
